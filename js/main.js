...
async function pullFromGitHub() {
    ...
    try {
        if (!githubAPI.hasToken()) {
            const token = prompt("Bitte geben Sie Ihr GitHub Personal Access Token ein:\n\n" +
                "Das Token wird sicher im Browser gespeichert.\n" +
                "Benötigte Berechtigung: repo (Full control of private repositories)");
        ...
        if (!result) {
            statusEl.textContent = "Keine creatures.json auf GitHub gefunden";
            statusEl.style.color = "red";
            
            if (confirm("Die Datei creatures.json existiert noch nicht auf GitHub.\nSoll sie mit den aktuellen Daten erstellt werden?")) {
                await pushToGitHub();
            }
            return;
        }

        creatureData = result.content;
        currentGitHubSHA = result.sha;
        
        populateHgFilter();
        populateTypeFilter();
        setupNewList();
        $("#out").innerHTML = `<div id="placeholder"><p>✅ ${creatureData.length} Kreaturen von GitHub geladen.</p></div>`;
        $("#file-name").textContent = `GitHub (${creatureData.length} Kreaturen)`;
        
        statusEl.textContent = "✅ Synchronisiert";
        statusEl.style.color = "green";
        
    } catch (error) {
        console.error("Pull-Fehler:", error);
        ...
        if (error.message.includes('401')) {
            ...
            alert(`Fehler beim Laden von GitHub:\n${error.message}`);
        }
    }
}
...
async function pushToGitHub() {
    ...
    if (creatureData.length === 0) {
        alert("Keine Kreaturen zum Hochladen vorhanden.");
        return;
    }
    ...
    const commitMessage = prompt(
        "Commit-Nachricht eingeben (optional):",
        `Update: ${creatureData.length} Kreaturen`
    );
    ...
    statusEl.textContent = "Lade zu GitHub hoch...";
    statusEl.style.color = "orange";
    
    try {
        if (!currentGitHubSHA) {
            const currentFile = await githubAPI.getFile();
            if (currentFile) {
                currentGitHubSHA = currentFile.sha;
            }
        }
        
        const result = await githubAPI.updateFile(
            creatureData,
            commitMessage || `Update: ${creatureData.length} Kreaturen`,
            currentGitHubSHA
        );
        
        currentGitHubSHA = result.content.sha;
        
        statusEl.textContent = "✅ Erfolgreich gepusht";
        statusEl.style.color = "green";
        
        alert(`✅ Erfolgreich zu GitHub gepusht!\n\n` +
              `${creatureData.length} Kreaturen wurden hochgeladen.\n` +
              `Commit: ${result.commit.sha.substring(0, 7)}`);
        
    } catch (error) {
        console.error("Push-Fehler:", error);
        statusEl.textContent = "❌ Push fehlgeschlagen";
        statusEl.style.color = "red";
        ...
        if (error.message.includes('401')) {
            ...
            alert("Konflikt: Die Datei wurde zwischenzeitlich geändert.\nBitte erst von GitHub laden (Pull) und dann erneut versuchen.");
            currentGitHubSHA = null;
        } else {
            alert(`Fehler beim Push zu GitHub:\n${error.message}`);
        }
    }
}
